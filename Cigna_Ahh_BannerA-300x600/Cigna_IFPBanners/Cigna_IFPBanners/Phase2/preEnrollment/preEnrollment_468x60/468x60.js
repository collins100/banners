...
function eventOne() {
 ...
 function eventSix() {
	...
	TweenLite.to(animatedCyanBox, 1, {y:"0", width:"366"}); //366
    TweenLite.delayedCall(3.5, eventSeven);
}
 function eventSeven() {
	TweenLite.to(frame6textA, 0.35, {opacity: "0", ease:Quad.easeIn});
	TweenLite.to(frame6textA, 1, {x:"469"});
	TweenLite.to(frame6textB, 0.35, {opacity: "0", ease:Quad.easeIn});
	TweenLite.to(frame6textB, 1, {x:"469"});
	TweenLite.to(frame7headA, 1.5, {opacity: "1", ease:Quad.easeIn, delay:.1});
	TweenLite.from(frame7headA, 0, {x:"-131", delay:.1});
	TweenLite.to(frame7headB, 1.5, {opacity: "1", ease:Quad.easeIn});
	TweenLite.from(frame7headB, 0, {x:"-131", delay:0});
...
function eventEight(){
	...
	TweenLite.to(animatedCyanBox, .5, {y:"0", width:"292", delay:.5}); //292
	TweenLite.to(frame7textA, .5, {opacity: "1", ease:Quad.easeIn, delay:0.5});
	TweenLite.from(frame7textA, 0, {x:"-131", delay:0});
	TweenLite.to(frame7textB, .5, {opacity: "1", ease:Quad.easeIn, delay:0.5});
	TweenLite.from(frame7textB, 0, {x:"-131", delay:0});
	TweenLite.to(frame7Logo, .2, {opacity: "1", ease:Quad.easeIn, delay:1});
	TweenLite.from(frame7Logo, 0, {y:"-100", delay:0}); //x350
	TweenLite.to(CTAbutton, .2, {opacity: "1", ease:Quad.easeIn, delay:1});
	TweenLite.from(CTAbutton, 0, {x:"300", delay:0});
	TweenLite.to(discImg, .2, {opacity: "1", ease:Quad.easeIn, delay:1});
	function showIt(){
	    TweenLite.to(discImg, .1, {css:{opacity:0}});
	    TweenLite.to(discText,.4, {css:{opacity:1}});

	}
	function hideIt(){
	    TweenLite.to(discText, .1, {css:{opacity:0}});
	    TweenLite.to(discImg,.4, {css:{opacity:1}});
